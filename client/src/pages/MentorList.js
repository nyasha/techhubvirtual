...
class MentorList extends Component {
  ...
  // Load data on start
  componentWillMount() {
    this.props.actionsMentor.loadMentorList();
  }
  ...
  confirmDialogDelete(id) {
    this.props.actionsMentor.deleteMentor(this.state.idDelete).then(data => {
      this.props.actionsMentor.loadMentorList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/mentors/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.MentorListReducer.listMentor
  };
}
